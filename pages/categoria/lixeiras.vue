<template>
  <div>
    <Categorias-Title>{{$t('bins')}}</Categorias-Title>
    ...
    <div v-else>
      <div class="grid grid-flow-row grid-cols-2 gap-y-4 mx-2 justify-items-center
        sm:grid-cols-3 sm:mx-8 sm:gap-y-6 lg:grid-cols-4 lg:gap-x-10
        xl:mx-12 xl:max-w-screen-xl xl:gap-x-20 2xl:mx-auto">
        <Card-Produto
...
export default {
  ...
  computed: {
    ...mapState({
      produtos: (state) => state.produtos.produtos.filter(item => item.subcategoria === 'Lixeira')
    }),
