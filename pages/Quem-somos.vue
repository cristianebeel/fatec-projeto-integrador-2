<template>
<section id="page">
  <section id="main">
  <Carousel />
</section>
  <Sobre />

<footer class="bg-red-300 w-full h-44 py-7 px-2 my-32 ">
        <div class=" md:py-8 px-4 pt-3 pb-4 border-b -mx-4 ">
            <div class="max-w-xl mx-auto">
              <br>
                <h2 class="text-xl text-left inline-block font-semibold leading-8 text-gray-900">{{$t('signUpNewsletter')}}</h2>
                <p class="text-gray-900 text-xs font-semibold pl-px">
                  {{$t('newsEmail')}}
                </p>
                <form action="#" class="mt-2">
                    <div class="flex items-center">
                        <input type="email" class="w-full px-2 py-4 mr-2  bg-red-100 shadow-inner rounded-md border border-gray-400 focus:outline-none" required>
                        <button class="bg-red-600 text-red-200 px-5 py-2 rounded shadow " style="margin-left: -7.8rem;">{{$t('signUp')}}</button>
                    </div>
                </form>
            </div>
        </div>
</footer>
</section>

</template>
